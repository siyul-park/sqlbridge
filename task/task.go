...
func (t *JoinTask) on(lhs, rhs schema.Record) (bool, error) {
	if t.On != nil {
		...
		val, err := vm.Eval(record, t.On, t.Args...)
		if err != nil {
			return false, err
		}
...
func (t *FilterTask) Run(ctx context.Context) (schema.Cursor, error) {
	...
	for i := 0; i < len(records); i++ {
		val, err := vm.Eval(records[i], t.Expr, t.Args...)
		if err != nil {
			return nil, err
		}
	...
	for i, record := range records {
		...
		for _, expr := range t.Exprs {
			...
			case *sqlparser.AliasedExpr:
				...
				val, err := vm.Eval(record, expr.Expr, t.Args...)
				if err != nil {
					return nil, err
				}
...
func (t *GroupTask) Run(ctx context.Context) (schema.Cursor, error) {
	...
	for _, record := range records {
		...
		for _, expr := range t.Exprs {
			...
			val, err := vm.Eval(record, expr, t.Args...)
			if err != nil {
				return nil, err
			}
...
func (t *OrderTask) Run(ctx context.Context) (schema.Cursor, error) {
	...
	for _, order := range t.Orders {
		sort.SliceStable(records, func(i, j int) bool {
			...
			val, err := vm.Eval(record, cmp, t.Args...)
			if err != nil {
				return false
			}
...
func (t *LimitTask) Run(ctx context.Context) (schema.Cursor, error) {
	...
	if t.Limit.Offset != nil {
		if val, err := vm.Eval(schema.Record{}, t.Limit.Offset, t.Args...); err != nil {
			return nil, err
	...
	if t.Limit.Rowcount != nil {
		if val, err := vm.Eval(schema.Record{}, t.Limit.Rowcount, t.Args...); err != nil {
			return nil, err
		} else if v := reflect.ValueOf(val); v.CanInt() {
			rowcount = int(v.Int())
		}
