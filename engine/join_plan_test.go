	...
	tests := []struct {
	...
	}{
		{
			plan: &JoinPlan{
				...
				Expr: &EqualExpr{
					Left:  &ColumnExpr{Value: &sqlparser.ColName{Name: sqlparser.NewColIdent("id"), Qualifier: sqlparser.TableName{Name: sqlparser.NewTableIdent("t1")}}},
					Right: &ColumnExpr{Value: &sqlparser.ColName{Name: sqlparser.NewColIdent("id"), Qualifier: sqlparser.TableName{Name: sqlparser.NewTableIdent("t2")}}},
				},
			},
